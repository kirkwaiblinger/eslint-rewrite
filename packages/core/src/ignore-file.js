 ...
 * @throws {Error} If the ignore file path is not an absolute path(s).
 */
export function includeIgnoreFileImpl(ignoreFilePath, options = {}) {
	if (!path.isAbsolute(ignoreFilePath)) {
		throw new Error(
			`The ignore file location must be an absolute path. Received ${ignoreFilePath}`,
		);
	}
	...
	return {
		name: options.name || "Imported .gitignore patterns",
		ignores: lines
			.map(line => line.trim())
			.filter(line => line && !line.startsWith("#"))
			.map(convertIgnorePatternToMinimatch),
		...(options.mode === "gitignore"
			? {
